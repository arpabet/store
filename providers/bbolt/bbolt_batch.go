...
// SetBatchRaw writes all entries in a single bbolt transaction (all-or-nothing).
func (t *implBoltStore) SetBatchRaw(ctx context.Context, entries []store.RawEntry) error {
	...
	err := t.db.Update(func(tx *bolt.Tx) error {
		for i := range entries {
			...
			// reads see this txn's own earlier writes, so duplicate keys version correctly
